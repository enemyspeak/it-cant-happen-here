...
-- config
arc.cfg = {
	msg_spd = 25, 		        -- chars/sec. message speed
	...
	msg_txt_shdw = false,	    -- boolean. true: use text shadow, false: don't use
    msg_nc_short_pause = 20,    -- number. number of characters wait for short pause
...
-- font
arc.fn = {}
arc.fn.f = lg.newFont("res/LetterGothicStd.otf")
...
-- images
arc.img = {
	cursor = lg.newImage('res/cursor.png'),
	blinker = lg.newImage('res/press.png')
}

-- sound effects
arc.sfx = {
	cursor = la.newSource('res/cursor.wav','static'),
	confirm = la.newSource('res/confirm.wav','static')
}
